...
def get_token(url, header, fixed_version, assigned_to):
    ...
    if url == 'http://120.25.239.138:18097/projects/ht_android/issues/new':  # 判断如果是新建bug页面,同时获取下X-CSRF-Token 用于上传文件
        ...
        assigned_to_id = extract_string(result, '<option value="(.*?)">'+assigned_to+'</option>')
...
def start_submit_bug(url, buildNewBugUrl, submitBugUrl, username, password, haveLog, logPath, title, contents, fixed_version, assigned_to):
    """
